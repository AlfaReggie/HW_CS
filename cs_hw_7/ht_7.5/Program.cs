...
double[,] getRandMatrix(int[] sizeArray)
{
    ...
    for(int idx = 0; idx < mtrx.GetLength(0); idx++)
    {
        for(int jdx = 0; jdx < mtrx.GetLength(1); jdx++)
        {
            ...
            double d = Math.Round(sizeArray[2] * 1.0 + rnd.NextDouble() * (sizeArray[3] * 1.0 - sizeArray[2] * 1.0), 3);
            mtrx[idx, jdx] = d;
        }
    }
    return mtrx;
...
int[] userArray(string[] array)
{
    ...
    {
        int nextEl = nextElArr($"Enter {array[idx]} size: ");
        array_01[idx] = nextEl;
    }
    return array_01;
}
...
while(true)
{
    ...
    {
        string[] usArrayStr = {"cols", "rows", "min border", "max border"};
        int[] usArrayInt = userArray(usArrayStr);
        
        double[,] matriX = getRandMatrix(usArrayInt);
        printMtrx(matriX);
    }    
}
